...
- 첫 실행에서는 메인 화면보다 온보딩을 먼저 표시
- 목표 메이트, 시작·회고 시간, 첫 목표를 차례로 설정
- 샘플 목표나 캘린더 기록 없이 온전히 나의 하루부터 시작
...
- 날짜가 바뀌어도 미회고 TODO를 별도 보관해 전날의 체크 상태가 사라지지 않음

### 카드형 하루 회고

- `오늘 해낸 일`과 `오늘 하기 어려웠던 일`을 카드로 한 장씩 확인
- 카드의 핵심 문구와 목표명을 한눈에 읽을 수 있도록 큰 글씨로 표시
- 어려웠던 일은 왼쪽으로 넘겨 그만하기, 오른쪽으로 넘겨 내일 이어하기
- 내일로 보낸 항목은 다음 날 자동으로 채우지 않고 사용자가 다시 선택
- 회고 마지막에는 목표 밖에서 해낸 일을 딱 한 가지만 입력하고 바로 다음 단계로 이동

### 목표 메이트

- 고양이, 강아지, 토끼, 곰 캐릭터 선택
- 설정에서 캐릭터를 작게·보통·크게 조절하거나 완전히 숨기기
- 데스크톱 위에서 드래그해 원하는 위치로 이동
- 여러 데스크톱을 사용해도 캐릭터에서 앱을 열면 현재 보고 있는 데스크톱에 표시
- 주기적으로 나타났다 사라지는 말풍선
- 그림자 없이 캐릭터 쪽으로 부드럽게 이어지는 말풍선 디자인
- 가끔 아직 완료하지 않은 실제 목표명을 언급하는 상황별 응원
- 이미 완료로 체크한 목표명은 이후 대사에서 다시 꺼내지 않음
...
- 전날 돌아보기를 놓치면 다음 실행에서 `지금 돌아보기` 또는 `기록만 남기기`를 선택
- 돌아보기를 건너뛰어도 완료·미완료 상태 그대로 전날 날짜에 기록
- 목표 밖에서 기억난 일은 별도 표시
- 목표 미작성과 회고 대기 상태를 메뉴 막대 아이콘의 서로 다른 점으로 표시
- 앱을 열지 않고도 메뉴 막대에서 오늘 화면, 회고, 기록, 설정으로 이동
- 설정의 `이메일로 버그 제보하기`에서 제보 양식이 채워진 메일 작성 창 열기

## 화면

![오늘도 첫 시작 온보딩](docs/images/onboarding.png)

<p align="center"><strong>샘플 데이터 없이 나의 리듬부터 고르는 첫 시작</strong></p>
...
1.20.1부터 앱 번들 전체를 패키징 마지막 단계에서 다시 봉인해, 이전 배포판이 일부 Mac에서 `손상되어 열 수 없음`으로 잘못 표시되던 문제를 수정했습니다.
