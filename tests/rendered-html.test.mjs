...
test("includes the native macOS companion and offline renderer", async () => {
  ...
  assert.match(page, /className="goal-drag-handle"/);
  assert.match(page, /moveGoalTo/);
  assert.match(page, /moveGoalBy/);
  assert.match(page, /onDragStart/);
  assert.match(page, /위아래 화살표 키를 사용하세요/);
  assert.match(styles, /\.goal-row\.is-dragging/);
  assert.match(styles, /\.goal-drag-handle/);
  ...
  assert.match(mateScript, /mateScale/);
  assert.match(main, /MATE_REFERENCE_DISPLAY = \{ width: 1920, height: 1243 \}/);
  assert.match(main, /display\.bounds\.width \/ MATE_REFERENCE_DISPLAY\.width/);
  assert.match(main, /display\.bounds\.height \/ MATE_REFERENCE_DISPLAY\.height/);
