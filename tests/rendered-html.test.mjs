...
test("includes the native macOS companion and offline renderer", async () => {
  ...
  assert.match(main, /MATE_COMPACT_SIZE = \{ width: 200, height: 200 \}/);
  assert.match(main, /MATE_SPEAKING_SIZE = \{ width: 268, height: 268 \}/);
  assert.match(main, /MATE_SIZE_SCALES = \{ small: 0\.8, medium: 1, large: 1\.2 \}/);
  assert.match(main, /MATE_CHARACTER_CENTER_INSET = 100/);
  assert.match(main, /screen\.getDisplayNearestPoint\(characterCenter\)/);
  assert.match(main, /workArea\.x \+ workArea\.width - next\.width/);
  assert.match(main, /function presentMainOnCurrentWorkspace/);
  assert.match(main, /setVisibleOnAllWorkspaces\(true, workspaceOptions\)/);
  assert.match(main, /setVisibleOnAllWorkspaces\(false, workspaceOptions\)/);
  assert.match(main, /skipTransformProcessType: true/);
  ...
  assert.match(mateStyles, /right:\s*12px/);
  assert.match(mateStyles, /bottom:\s*12px/);
  assert.match(mateStyles, /pointer-events:\s*none/);
  assert.doesNotMatch(mateStyles, /rotate\(-10\.5deg\)/);
  assert.match(mateStyles, /font-size:\s*28px/);
  assert.match(mateStyles, /PretendardVariable\.woff2/);
  assert.match(mateStyles, /--progress-color/);
  assert.match(mateStyles, /--mate-scale/);
  assert.match(mateStyles, /scale\(var\(--mate-scale\)\)/);
  assert.doesNotMatch(mateStyles, /drop-shadow/);
  assert.doesNotMatch(mate, /<small>오늘<\/small>/);
  assert.match(mateStyles, /\.character:focus-visible[\s\S]*outline:\s*none/);
  ...
  assert.match(mateScript, /`\$\{completed\} \/ \$\{total\}`/);
  ...
  assert.match(mateScript, /MATE_SCALES/);
  assert.match(mateScript, /characterSize = "medium"/);
  ...
  assert.match(page, /oneuldo:set-character-visibility/);
  assert.match(styles, /Desktop companion visibility and size controls/);
  assert.match(styles, /\.topbar\s*\{[^}]*position:sticky;[^}]*top:0;[^}]*z-index:60/);
  assert.match(styles, /\.mate-status strong\{[^}]*font-weight:950;[^}]*text-stroke:\.55px currentColor/);
  assert.match(page, /extraDone\?: string\[\]/);
  assert.match(page, /오늘 기억하고 싶은 일/);
  assert.match(page, /딱 하나만 남겨볼까요/);
  assert.match(page, /한 가지면 충분해요/);
  assert.match(page, /if \(!title \|\| rememberedDone\) return/);
  assert.match(page, /setMemoryFinished\(true\)/);
  assert.match(page, /이 한 가지 기록하기/);
  ...
  assert.doesNotMatch(page, /const DEFAULT_GOALS/);
  assert.doesNotMatch(page, /const SAMPLE_RECORDS/);
  assert.match(page, /removeLegacySampleGoals/);
  assert.match(page, /removeLegacySampleRecords/);
  assert.match(styles, /\.app-shell\.is-first-onboarding>:not\(\.onboarding-backdrop\)/);
